defmodule Anoma.Node.Examples.EShardSupervisor do
  ...
  ShardSupervisor starts the correct Shard processes,
  ...
  def test_shard_supervisor_startup_and_routing() do
    ...
    # 2. Verify Shard Processes Exist
    ...
    # 4. Query the ETS table directly for key -> shard_label mapping
    # Note: Accessing the private @shard_key_map_ets is not ideal, but necessary for this test setup.
    table_name = :"shard_key_map_#{node_id}"

    assert :ets.lookup(table_name, "a") == [{"a", :a}],
           "lookup for 'a' failed"

    assert :ets.lookup(table_name, "b") == [{"b", :b}],
           "lookup for 'b' failed"

    assert :ets.lookup(table_name, "c") == [{"c", :c}],
           "lookup for 'c' failed"

    assert :ets.lookup(table_name, "d") == [],
           "lookup for unknown key 'd' should return :error"
